...
When enabled (Settings → Tools → **Web search & read**), the Familiar gains three tools: `look_up` answers a definition/fact/overview question from keyless official reference APIs (Wikipedia + the DuckDuckGo Instant Answer API — no scraping, no setup); `web_search` finds pages out on the web; and `read_webpage` opens a result and reads it as clean markdown. It works out of the box with a built-in keyless backend — nothing to install. `read_webpage` is SSRF-guarded (public http/https only — loopback, private, link-local, and cloud-metadata targets are refused), times out on slow hosts, and stamps each read with its source URL and date. A page stays in the conversation for the session; the Familiar keeps anything worth remembering past it via `save_to_tome`. **Finding pages** (`web_search`) works out of the box (a built-in keyless backend) and can be upgraded via **Settings → Web search → Configure search backend**: a proper search **API** (Tavily — no card; Brave; or Google), or a **local engine** the Familiar installs and runs on your machine — **LibreY** (lightest), **4get** (medium), or **SearXNG** (heaviest, broadest). The local engines are fetched + run on first use (Linux/macOS; the PHP ones need no manual PHP install — a static PHP runtime is fetched too). Whatever you pick, a wrong key or a down engine always falls back to the built-in search — you're never left without it. Power users can also point search at their own [SearXNG](https://docs.searxng.org/) — see [Web search setup](websearch-setup.md). Not sure which to pick? The modal has a **"Ask me about these options"** mini-chat — the same Familiar, in its own voice, explaining the trade-offs and walking you through signup in plain language. Hard-disable web access with `PROTO_FAMILIAR_WEBSEARCH_DISABLED=1`; the managed local engines with `PROTO_FAMILIAR_LOCAL_ENGINE_DISABLED=1`; the in-modal guide with `PROTO_FAMILIAR_GUIDE_CHAT_DISABLED=1`.
