...
work-issued laptops). The launcher (`Familiar.vbs`) detects this
...
- Ask IT to allow PowerShell scripts in the Familiar folder.
- Install on a personal machine; copy the working install over later.

### "Familiar is under OneDrive — relocate?"
...
`%LOCALAPPDATA%\Familiar` (outside OneDrive, short path,
...
"install Familiar on a non-corporate machine" path.
...
Cause: a previous `node.exe` from Familiar is still running.
...
the port owner if it looks like Familiar.
...
look like Familiar", another app on your machine grabbed the
...
identity layer — as a child process on Familiar startup; if it
...
**Fix:** restart Familiar (`npm start`). On startup, watch the
...
- **Phylactery skipped at startup** — usually because `uv` isn't
  ...
  `Familiar.vbs`), or — if uv is already installed — run
  `cd phylactery && uv sync` manually, then restart Familiar.
...
- **Repeated reconnect attempts** — the venv exists so the skip-check
  ...
  path), then restart Familiar to retry — the loop doesn't
  re-arm itself once it's given up.

Phylactery degrades to absent without breaking chat: if it can't
start, enrichment is skipped and Familiar runs normally.
...
expected. Update Familiar; the existing files need no migration.
...
(default 30 days). Bump that env var before starting Familiar
...
**Fix:** open Familiar's **Connections** sidebar, click
...
the Phylactery child with the new env — no Familiar restart
...
process (or `phylactery/.venv` isn't synced). Restart Familiar
...
the rest of Familiar boots and runs without it. You'll just
...
(or `Familiar.vbs`) and the next launch will find it.
...
- After 10 attempts the loop gives up — restart Familiar to
  retry.
...
> **Updating does not invalidate Discord tokens.** Pulling new Familiar
...
- **You installed from a downloaded ZIP, not a `git clone`.** GitHub's "Download ZIP" gives you a folder like `Familiar-main` that is **not** a git repository — there's no `.git` inside it. The installer pulls updates with `git`, so on a ZIP it silently can't, and you stay on whatever version the ZIP captured. (The "up to date" you saw may also have been **npm's** `up to date` line, which is about node modules, not the app.) The installer now detects this, warns explicitly, and prints `Branch: (not a git checkout …)` at the end.
  ...
  **Or reinstall via git** (enables the installer's own `git pull` going forward):
  ...
  ```
  git clone https://github.com/ScarletPrinceEury/Familiar.git
  ```
...
- **In the app:** the **version badge in the sidebar footer** reads `Familiar vX.Y.Z`.
- **Endpoints:** open `http://localhost:8742/api/version` or `/api/health` in a browser.
- **At install time:** the installer now prints `Version: Familiar vX.Y.Z` and the git `Branch:` when it finishes.
...
the launcher scripts both auto-recycle Familiar's *own* stale
instances, but they refuse to kill anything else.

**If the holder IS a stale Familiar:** the prestart message
...
`port X is held by PID Y, which isn't a Familiar instance` —
