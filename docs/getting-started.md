...
| Action | Result |
|---|---|
| Left-click | Open Familiar in your browser |
...
2. Double-click **`Familiar.command`** in Finder.
...
4. A Terminal window opens showing server logs; your browser opens automatically at `http://localhost:8742`. The launcher auto-recycles any stale Familiar holding the port before binding.

**To shut down**, press **Ctrl-C** in the Terminal window, then close it (Cmd-W). Because `node` runs in the foreground, Ctrl-C cleanly stops Familiar, Phylactery, and Unruh.

> If macOS Gatekeeper warns about an unidentified developer, right-click `Familiar.command` → **Open** the first time.

### Linux

1. Clone the repo.
2. Run `./install.sh` once. It checks Node, auto-installs uv via its official one-liner installer (`curl … | sh`) if missing, runs `npm install`, sets up the in-tree Phylactery + Unruh Python venvs via `uv sync`, and registers a `.desktop` entry under `~/.local/share/applications/` so **Familiar** appears in your app launcher / activities overview. Re-runnable — the desktop entry is created only if it doesn't already exist.
...
```bash
git clone https://github.com/ScarletPrinceEury/Familiar
cd Familiar
npm install
...
Windows equivalents (`install.bat`, `start.bat`, `stop.bat`, and the PowerShell installer under `scripts/win/install.ps1`) behave identically — `install.bat` detects `node_modules\` and routes to update mode the same way, auto-installs prereqs via winget (with a PowerShell-installer fallback for uv when winget is missing), and creates Desktop + Start Menu shortcuts idempotently. The recommended Windows entry point is `Familiar.vbs` — it avoids the brief console flash and gives you the tray icon, and re-runs the installer whenever `node_modules\` or the Python venvs are missing.

`npm start` and `npm run dev` are also valid entry points. They run two prestart hooks (`scripts/ensure-unruh-deps.mjs` and `scripts/ensure-port-free.mjs`) that sync the Unruh venv and recycle any stale Familiar before binding, so launching this way has the same auto-recovery behaviour as the launcher scripts.
...
2. **`git pull --ff-only`** on the Familiar repo. Skipped if the directory isn't a git checkout. The `--ff-only` flag means git refuses any non-fast-forward merge — if you're on a non-default branch, have local commits, or have uncommitted changes that would conflict, the pull aborts with a warning and the work tree is left exactly as you had it.
...
On Windows, set the env var in the current shell before double-clicking `Familiar.vbs`, or `setx PORT 8080` and start a new session.

## Access from other devices (Tailscale / LAN)

By default Familiar is reachable only from the machine it's running on. To use the UI from your phone, tablet, or another laptop:

1. Open Familiar in your browser (on the machine running the server).
...
Familiar always binds to `0.0.0.0`, but until the toggle is on a middleware rejects every non-loopback request with a 403. The effective behaviour with the toggle off matches the historical `localhost`-only bind — nothing on the network can actually talk to the server.
...
That exposes the loopback server over HTTPS to your tailnet without changing Familiar's gate. See [Tailscale Serve docs](https://tailscale.com/kb/1242/tailscale-serve).
...
Phylactery's own env vars (read by Phylactery itself, not by Familiar; documented for completeness because the **+ Phylactery** connection designation sets them automatically):

| Variable | Set by Familiar | Purpose |
|---|---|---|
...
In short: Phylactery lives at `./phylactery/` inside the Familiar repo, and `thalamus.js` spawns it on startup as a Python MCP child via `uv run` (exactly the way it spawns Unruh). If `uv` isn't installed or the `phylactery/.venv` hasn't been synced, Phylactery is treated as absent — enrichment is skipped and Familiar runs normally. Hard-disable it with `PROTO_FAMILIAR_PHYLACTERY_DISABLED=1`.

---

## Versioning

Familiar's version lives in `package.json` (`version` field) and is the **single source of truth**. The server reads it at boot and exposes it via:

- `/api/version` → `{ "version": "<v>" }`
- `/api/health`  → `{ "ok": true, "version": "<v>" }`
- The startup banner: `Familiar <v> running at:`
...
| Windows: `node` not on PATH after install | Close the install window, open a new shell (so PATH refreshes), and double-click `Familiar.vbs` again. |
| Windows: SmartScreen blocks the `.vbs` | Click **More info → Run anyway**. The script is plain text — feel free to read it first. |
| Windows: shortcuts not created on a fresh install | Re-run `Familiar.vbs` — shortcut creation is idempotent and creates each `.lnk` only if it doesn't already exist (so re-running picks up any that are missing without touching the ones that are present). |
| macOS: "unidentified developer" warning | Right-click `Familiar.command` → **Open** the first time. |
...
| `[thalamus] Unruh venv missing` warning | Re-run the installer (`./install.sh`, `install.bat`, or just `Familiar.vbs`) — or, if uv is already installed, run `cd unruh && uv sync` manually. Familiar boots and runs without Unruh, just without the temporal-context block. |
